...
def upload_track(user_id: int, file_path: str) -> dict:
    """
    POST /api/tracks — upload an audio file (MP3 or WAV) linked to user_id.
...
def normalize_track(track_raw: dict, analysis_raw=None) -> dict:
    ...
    return {
        ...
        "key":    a.get("key") or track_raw.get("key") or "—",
        "status": a.get("status") or track_raw.get("status") or "pending",
    }